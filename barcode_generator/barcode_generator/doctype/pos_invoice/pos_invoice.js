...
erpnext.selling.POSInvoiceController = class POSInvoiceController extends erpnext.selling.SellingController {
	...
	onload(doc) {
		...
		this.frm.ignore_doctypes_on_cancel_all = [
			...
			"POS Closing Entry",
			// Removed "Serial and Batch Bundle" to disable serial number handling
		];
		...
		this.frm.set_query("item_code", "items", function (doc) {
			return {
				...
				filters: {
					...
					// Keep serialized items but handle serial numbers elsewhere
				},
			};
		});
...
// Override item row events to disable serial number requirements in POS
frappe.ui.form.on("POS Invoice Item", {
	item_code: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		// Clear serial number fields but allow the item to be selected
		frappe.model.set_value(cdt, cdn, "serial_and_batch_bundle", "");
		frappe.model.set_value(cdt, cdn, "serial_no", "");
		frappe.model.set_value(cdt, cdn, "batch_no", "");
		
		// Disable serial number requirement for this transaction
		if (row.item_code) {
			frappe.db.get_value("Item", row.item_code, "has_serial_no", function(r) {
				if (r && r.has_serial_no) {
					// Item has serial no but we're skipping it for POS
					console.log("Serial number tracking disabled for POS Invoice: " + row.item_code);
				}
			});
		}
	},
	...
	qty: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		// Clear serial number fields when quantity changes
		frappe.model.set_value(cdt, cdn, "serial_and_batch_bundle", "");
		frappe.model.set_value(cdt, cdn, "serial_no", "");
		frappe.model.set_value(cdt, cdn, "batch_no", "");
	},
	
	// Override any automatic serial number population
	serial_and_batch_bundle: function(frm, cdt, cdn) {
		// Always clear this field in POS
		frappe.model.set_value(cdt, cdn, "serial_and_batch_bundle", "");
	},
