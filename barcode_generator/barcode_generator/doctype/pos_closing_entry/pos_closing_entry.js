...
frappe.ui.form.on("POS Closing Entry", {
	onload: async function (frm) {
		...
		frm.set_query("user", function (doc) {
			return {
				query: "barcode_generator.barcode_generator.doctype.pos_closing_entry.pos_closing_entry.get_cashiers",
    ...
    if (frm.doc.pos_opening_entry && frm.doc.period_start_date && 
        frm.doc.period_end_date && frm.doc.user) {
        
        reset_values(frm);
        const is_cashier = await is_cashier_profile(frm.doc.pos_profile);
        
        if (is_cashier) {
            // Cashier: fetch ALL invoices for the day
            frappe.run_serially([
                () => frappe.dom.freeze(__("Loading All Invoices! Please Wait...")),
                () => frm.trigger("set_opening_amounts"),
                () => frm.trigger("get_all_pos_invoices"), // New method
                () => frm.trigger("get_all_sales_invoices"), // New method  
                () => frappe.dom.unfreeze(),
            ]);
        } else {
            // Waiter: only set opening amounts, no invoice fetching
            frappe.run_serially([
                () => frappe.dom.freeze(__("Setting Opening Amounts...")),
                () => frm.trigger("set_opening_amounts"),
                () => frappe.dom.unfreeze(),
            ]);
        }
	...
	get_pos_invoices(frm) {
		return frappe.call({
			method: "barcode_generator.barcode_generator.doctype.pos_closing_entry.pos_closing_entry.get_pos_invoices",
			args: {
    ...
    get_all_pos_invoices(frm) {
    return frappe.call({
        method: "barcode_generator.barcode_generator.doctype.pos_closing_entry.pos_closing_entry.get_all_pos_invoices_for_day",
        args: {
            start: frappe.datetime.get_datetime_as_string(frm.doc.period_start_date),
            end: frappe.datetime.get_datetime_as_string(frm.doc.period_end_date),
            // Note: No pos_profile or user filter - get ALL for the day
        },
        callback: (r) => {
            let pos_docs = r.message;
            set_pos_transaction_form_data(pos_docs, frm);
...
get_all_sales_invoices(frm) {
    return frappe.call({
        method: "barcode_generator.barcode_generator.doctype.pos_closing_entry.pos_closing_entry.get_all_sales_invoices_for_day", 
        args: {
            start: frappe.datetime.get_datetime_as_string(frm.doc.period_start_date),
            end: frappe.datetime.get_datetime_as_string(frm.doc.period_end_date),
            // Note: No pos_profile or user filter - get ALL for the day
	...
	get_sales_invoices(frm) {
		return frappe.call({
			method: "barcode_generator.barcode_generator.doctype.pos_closing_entry.pos_closing_entry.get_sales_invoices",
			args: {
				start: frappe.datetime.get_datetime_as_string(frm.doc.period_start_date),
				end: frappe.datetime.get_datetime_as_string(frm.doc.period_end_date),
				pos_profile: frm.doc.pos_profile,
...
before_save: async function (frm) {
    ...
    if (!is_cashier) {
        // Waiter: Skip invoice processing entirely
        return;
    }
    
    // Cashier: Process ALL invoices for the day
    frappe.dom.freeze(__("Processing All Sales! Please Wait..."));
